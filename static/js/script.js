					...
					function generateButtons(user, productId) {
						if (!user.is_authenticated) {
							return `
                                <div class="text-center mt-6">
                                    <div class="flex justify-center space-x-4">
                                        <a href="login" class="flex-1 bg-blue-600 text-white py-2 px-4 rounded hover:bg-blue-700 text-center whitespace-nowrap">Pieslēgties</a>
                                        <a href="register" class="flex-1 bg-green-600 text-white py-2 px-4 rounded hover:bg-green-700 text-center whitespace-nowrap">Reģistrēties</a>
                                    </div>
                                </div>
                            `;
							...
							return `
                                <div class="text-center mt-6">
                                    <div class="flex justify-center space-x-4">
                                        <a href="/plans" class="flex-1 bg-blue-500 text-white font-bold py-2 px-4 rounded hover:bg-blue-700 text-center whitespace-nowrap">Pirkt</a>
                                        <span class="flex-1 bg-green-500 text-white py-2 px-4 rounded hover:bg-green-700 cursor-pointer promo-code-link text-center whitespace-nowrap" data-id="${productId}">Ir kods?</span>
                                    </div>
                                </div>
                            `;
							...
							if (user.downloads_remaining > 0) {
								return `
                                    <button class="download-btn w-full text-center py-2 px-4 bg-blue-600 text-white rounded cursor-pointer mt-6 hover:bg-blue-700" data-id="${productId}">Lejuplādēt</button>
                                `;
							} else {
								return `
                                    <button class="w-full text-center py-2 px-4 bg-blue-600 text-white rounded cursor-pointer mt-6 hover:bg-blue-700 promo-code-link" data-id="${productId}">
                                        Ievadīt promo kodu
                                    </button>
                                `;
							}
